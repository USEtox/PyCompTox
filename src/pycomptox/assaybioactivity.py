...
class AssayBioactivity:
    ...
    def _make_request(
        ...
        try:
            ...
            if response.status_code == 400:
                raise ValueError(f"Invalid request or projection type")
            elif response.status_code == 404:
                raise ValueError(f"No data found for the provided identifier")
            else:
    ...
    def get_assay_endpoints_list(self, gene_symbol: str) -> List[Dict[str, Any]]:
        """
    ...
    def get_count_available_assays(self) -> int:
        """
    ...
    def get_all_assays(self, projection: str = "assay-all") -> List[Dict[str, Any]]:
        """
    ...
    def find_assay_annotations_by_aeid_batch(
        ...
        try:
            response = requests.post(url, headers=headers, json=aeid_list)
            ...
            if response.status_code == 400:
                raise ValueError(f"Invalid request or AEID format")
            elif response.status_code == 404:
                raise ValueError(f"No data found for the provided AEIDs")
            else:
                raise RuntimeError(f"HTTP error occurred: {e}")
        except requests.exceptions.RequestException as e:
            raise RuntimeError(f"Request failed: {e}")
